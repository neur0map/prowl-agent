...
| C4 | Persistent Job Status, authenticated fetch stream, canonical refetch | `web/src/transport/events.ts`, `web/src/features/jobs/JobStatus.tsx`, App/API/styles | Replay/retry/reset/cancel UX | Historical H-C4 passed; F-BROWSER compiled recovery journey passed. | Bearer remains module memory only; events are invalidations, not durable truth. | Reconnect/reset observed final; final reviews pending. |
