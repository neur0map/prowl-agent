...
func Register(root *cobra.Command, version string) {
	root.AddCommand(newInitCmd(), newOpenCmd(defaultOpenDependencies()), newStatusCmd(version), newDoctorCmd(), newKnowledgeCmd(), newContextCmd(), newCapabilitiesCmd(), newSessionCmd(), newServeCmd(version), newLSPCmd(version), newUpdateCmd(version), newRestartCmd(version), newVersionCmd(version))
