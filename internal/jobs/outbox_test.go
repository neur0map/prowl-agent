...
func TestOutboxTransactionCreatesChangedEvent(t *testing.T) {
	...
	if err := store.AdvanceRetention(context.Background(), rows[0].Sequence+1, "snapshot://jobs"); err != nil {
		t.Fatal(err)
	}
	...
	if state.RetentionFloor != rows[0].Sequence+1 || state.SnapshotURI != "snapshot://jobs" {
