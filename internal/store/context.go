...
func (s *Store) FirstChunk(relPath string) (ChunkBody, bool, error) {
	var chunk ChunkBody
	err := s.db.QueryRow(`SELECT f.rel_path,c.start_line,c.text FROM chunks c JOIN files f ON f.id=c.file_id WHERE f.rel_path=? ORDER BY c.start_line LIMIT 1`, relPath).
